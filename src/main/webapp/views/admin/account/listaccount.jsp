					...
					<div class="card mb-4">
						...
						<div class="card-body">
							<table id="datatablesSimple">
								...
								<tbody>
									<c:forEach items="${accounts1}" var="i">
										<tr>
											<td>${i.email}</td>
											<td>${i.fullName}</td>
											<td>${i.image}</td>
											...
											<td><c:choose>
													<c:when test="${i.status == 1}">
														<form action="/Nhom11/admin/accounts/delete?id=${ i.id.toString() }" method="post"> <button type="submit">Delete</button> </form>
													</c:when>
													<c:when test="${i.status == 0}">
														<form action="/Nhom11/admin/accounts/restore?id=${ i.id.toString() }" method="post"> <button type="submit">Restore</button> </form>
													</c:when>
												</c:choose></td>
										</tr>
									</c:forEach>

								...
								<tbody>
									<c:forEach items="${Products}" var="i">
										<tr>
											...
											<td>${i.name}</td>
											<td>${i.image}</td>
											<td><fmt:formatNumber value="${i.unitPrice}"
													type="number" pattern="###" /> VNĐ</td>
											<td>${i.description}</td>
											<td>${i.quantityLeft}</td>
											<td>${i.supplier.name}</td>
											<td>${i.category.name}</td>
											<td>${i.status}</td>
											<td><a href="/Nhom11/admin/products/update?id=${ i.id.toString() }">Chỉnh sửa</a></td>
											<td><c:choose>
													<c:when test="${i.status == 1}">
														<form action="/Nhom11/admin/products/delete?id=${ i.id.toString() }" method="post"> <button type="submit">Delete</button> </form>
													</c:when>
													<c:when test="${i.status == 0}">
														<form action="/Nhom11/admin/products/restore?id=${ i.id.toString() }" method="post"> <button type="submit">Restore</button> </form>
													</c:when>
												</c:choose></td>
										</tr>
									</c:forEach>
